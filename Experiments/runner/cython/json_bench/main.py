...
import random
import string
from json_cy import create_complex_data_cy, json_dumps_cy, json_loads_cy
...
def benchmark_json_io(num_records, num_runs=5):
    """
    Benchmarks JSON encoding (dumps) and decoding (loads) performance using Cython optimizations.
    ...
    """
    # 1. Generate the initial complex Python object once (using Cython-optimized function)
    py_data = create_complex_data_cy(num_records)
    
    # Warm-up run
    warmup_json = json_dumps_cy(py_data)
    json_loads_cy(warmup_json)
    ...
    return {
        "algorithm": "JSON Encode/Decode (Cython)",
