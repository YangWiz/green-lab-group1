import time
import random
from conv_cy import convolution_1d_cy, convolution_2d_cy
...
def benchmark_convolution(data_size, kernel_size, num_runs=5):
    """Benchmarks 1D and 2D convolution (Cython optimized) for given data sizes and kernel sizes."""
    ...
    total_time_1d = 0
    # Warmup
    convolution_1d_cy(data_1d[:int(data_size * 0.1)], kernel_1d) 
    for _ in range(num_runs):
        start_time = time.perf_counter()
        convolution_1d_cy(data_1d, kernel_1d)
        end_time = time.perf_counter()
        total_time_1d += (end_time - start_time)
        
    results['1D'] = {
        'algorithm': "Convolution 1D (Cython)",
    ...
    if image_dim > 10:
        warmup_size = 10
        convolution_2d_cy([row[:warmup_size] for row in data_2d[:warmup_size]], kernel_2d) 
    
    for _ in range(num_runs):
        start_time = time.perf_counter()
        convolution_2d_cy(data_2d, kernel_2d)
        end_time = time.perf_counter()
        total_time_2d += (end_time - start_time)

    results['2D'] = {
        'algorithm': "Convolution 2D (Cython)",
