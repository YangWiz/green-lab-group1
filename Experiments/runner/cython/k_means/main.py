...
import random
import math
from kmeans_cy import initialize_data_cy, initialize_centroids_cy, kmeans_iteration_cy

# --- K-means Implementations ---

def _euclidean_distance(p1, p2):
    """Calculates the Euclidean distance between two D-dimensional points."""
    # Assumes p1 and p2 have the same number of dimensions (D)
...
def kmeans_iteration(data, centroids):
    ...
    # clusters[k] stores a list of data points assigned to centroid k
    ...
    for k in range(K):
        ...
        if not cluster_points:
            ...
            # For simplicity in benchmarking, we keep the old one.
...
# --- K-means Benchmarking Function ---

def benchmark_kmeans(N, D, K, num_runs=5):
    """
    Benchmarks the Cython-optimized K-means iteration algorithm.
    ...
    initial_data = initialize_data_cy(N, D)
    
    # Generate unique starting centroids for each run
    initial_states = [
        (initial_data, initialize_centroids_cy(initial_data, K))
    ...
    warmup_data = initialize_data_cy(N_warmup, D_warmup)
    warmup_centroids = initialize_centroids_cy(warmup_data, K_warmup)
    kmeans_iteration_cy(warmup_data, warmup_centroids)
    
    for data, centroids in initial_states:
        start_time = time.perf_counter()
        kmeans_iteration_cy(data, centroids)
    ...
    return {
        "algorithm": "K-means Single Iteration (Cython)",
        "N": N,
        "D": D,
        "K": K,
        "complexity": "O(N * K * D)",
...
if __name__ == "__main__":
    ...
    print("--- K-means Single Iteration Benchmark Test Runs ---")
