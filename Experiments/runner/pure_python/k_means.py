...
def benchmark_kmeans(N, D, K, num_runs=5):
    """
    Benchmarks the K-means iteration algorithm.
    ...
    initial_data = initialize_data(N, D)
    
    # Generate unique starting centroids for each run
    initial_states = [
        (initial_data, initialize_centroids(initial_data, K))
    ...
    warmup_data = initialize_data(N_warmup, D_warmup)
    warmup_centroids = initialize_centroids(warmup_data, K_warmup)
    kmeans_iteration(warmup_data, warmup_centroids)
    
    for data, centroids in initial_states:
        start_time = time.perf_counter()
        kmeans_iteration(data, centroids)
    ...
    return {
        "algorithm": "K-means Single Iteration",
        "N": N,
        "D": D,
        "K": K,
        "complexity": "O(N * K * D)",
